...
// XXX ugh, this defeats 'continue' & etc.
#define LCM_THRD_TRY(expr, recover)   \
    do {                              \
        if ((expr) != thrd_success) { \
            perror(__FILE__ #expr);   \
            recover                   \
        }                             \
    } while (0)
...
struct _lcm_provider_t {
    ...
    mtx_t mutex; /* Recursive. Must be locked when reading/writing to the above three queues */
    ...
    mtx_t transmit_lock;  // so that only thread at a time can transmit

    /* synchronization variables used only while allocating receive resources
     */
    int creating_read_thread;
    cnd_t create_read_thread_cond;
    mtx_t create_read_thread_mutex;
    mtx_t *p_create_read_thread_mutex;
...
static void lcm_udpm_destroy(lcm_udpm_t *lcm)
{
    ...
    mtx_destroy(&lcm->mutex);
    mtx_destroy(&lcm->transmit_lock);
    if (lcm->p_create_read_thread_mutex) {
        mtx_destroy(&lcm->create_read_thread_mutex);
        lcm->p_create_read_thread_mutex = NULL;
        cnd_destroy(&lcm->create_read_thread_cond);
    }
    free(lcm);
}
...
static int _recv_message_fragment(lcm_udpm_t *lcm, lcm_buf_t *lcmb, uint32_t sz)
{
    ...
    if (0 == fbuf->fragments_remaining) {
        ...
        int mtx_result = mtx_lock(&lcm->mutex);
        lcm_buf_free_data(lcmb, lcm->ringbuf);
        mtx_result = mtx_unlock(&lcm->mutex);
...
static lcm_buf_t *udp_read_packet(lcm_udpm_t *lcm)
{
    ...
    int mtx_result = mtx_unlock(&lcm->mutex);
    ...
    */
    ...
    while (!got_complete_message) {
        ...
        if (!lcmb) {
            LCM_THRD_TRY(mtx_lock(&lcm->mutex), {
		// lbmb is null and nothing else needs cleanup.
                return NULL;
            });
            lcmb = lcm_buf_allocate_data(lcm->inbufs_empty, &lcm->ringbuf);
            LCM_THRD_TRY(mtx_unlock(&lcm->mutex), {
                free(lcmb);
                return NULL;
            });
        }
                            ...
                            unsigned int datalen)
{
    ...
    if (payload_size <= LCM_SHORT_MESSAGE_MAX_SIZE) {
        ...
        int mtx_result = mtx_lock(&lcm->transmit_lock);
        ...
        lcm->msg_seqno++;
        mtx_result = mtx_unlock(&lcm->transmit_lock);
...
static int _setup_recv_parts(lcm_udpm_t *lcm)
{
    ...
    if (lcm->creating_read_thread) {
        ...
        int mtx_result = mtx_lock(lcm->p_create_read_thread_mutex);
        mtx_result = mtx_unlock(&lcm->mutex);

        // wait for the thread creating the read thread to finish
        while (lcm->creating_read_thread) {
            cnd_wait(&lcm->create_read_thread_cond, lcm->p_create_read_thread_mutex);
        }
        mtx_result = mtx_unlock(lcm->p_create_read_thread_mutex);
        mtx_result = mtx_lock(&lcm->mutex);
    ...
    mtx_result = mtx_init(&lcm->create_read_thread_mutex, mtx_plain);
    if (mtx_result != thrd_success) {
        perror(__FILE__ " mtx_init(create_read_thread_mutex)");
        mtx_result = mtx_unlock(&lcm->mutex);
        return 0;
    }
    lcm->p_create_read_thread_mutex = &lcm->create_read_thread_mutex;
    mtx_result = cnd_init(&lcm->create_read_thread_cond);
    if (mtx_result != thrd_success) {
        perror(__FILE__ " cond_init)");
        mtx_result = mtx_unlock(&lcm->mutex);
        return 0;
    }
...
/* Enable per-packet timestamping by the kernel, if available */
    ...
    mtx_result = mtx_lock(lcm->p_create_read_thread_mutex);
    lcm->creating_read_thread = 0;
    mtx_result = cnd_broadcast(&lcm->create_read_thread_cond);
    mtx_result = mtx_unlock(lcm->p_create_read_thread_mutex);
    mtx_result = mtx_unlock(&lcm->mutex);
...
setup_recv_thread_fail:
    ...
    LCM_THRD_TRY(mtx_unlock(&lcm->mutex),
                 {
                     // We're already in the error path; don't do any further handling.
                 });
    return -1;
}
...
static lcm_provider_t *lcm_udpm_create(lcm_t *parent, const char *network, const GHashTable *args)
{
    ...
    LCM_THRD_TRY(mtx_init(&lcm->mutex, mtx_plain | mtx_recursive), {
        lcm_udpm_destroy(lcm);
        return NULL;
    });
    LCM_THRD_TRY(mtx_init(&lcm->transmit_lock, mtx_plain), {
        lcm_udpm_destroy(lcm);
        return NULL;
    });
